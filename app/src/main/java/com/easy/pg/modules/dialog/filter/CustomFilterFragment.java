...
import com.easy.pg.modules.dialog.BottomSheetDialog;
import com.easy.pg.modules.dialog.BottomViewPagerBehaviour;
import com.easy.pg.modules.dialog.CustomBottomSheetDialogFragment;
import com.easy.pg.modules.dialog.DialogUtils;

public class CustomFilterFragment extends CustomBottomSheetDialogFragment{
    ...
    private BottomViewPagerBehaviour mBottomSheetBehavior;
    ...
    private BottomViewPagerBehaviour.BottomSheetCallback mBottomSheetBehaviorCallback = new BottomViewPagerBehaviour.BottomSheetCallback() {

        @Override
        public void onStateChanged(View bottomSheet, int newState) {
            switch (newState) {
                case BottomViewPagerBehaviour.STATE_HIDDEN:
                    if (callbacks != null) {
                        callbacks.onResult(FilterConstants.SWIPE_DOWN);
                    }
                    dismiss();
                    break;
                case BottomViewPagerBehaviour.STATE_COLLAPSED:
                    ViewGroup.LayoutParams params = view_main.getLayoutParams();
                    params.height = ViewGroup.LayoutParams.MATCH_PARENT;
                    view_main.setLayoutParams(params);
                    break;
                case BottomViewPagerBehaviour.STATE_EXPANDED:
...
//        super.setupDialog(dialog, style);
        if (viewPager != null) {
            DialogUtils.setupViewPager(viewPager);
        }
        ...
        mBottomSheetBehavior = BottomViewPagerBehaviour.from(((View) contentView.getParent()));
        ...
        dialog.setOnShowListener(new DialogInterface.OnShowListener() {
            ...
            public void onShow(DialogInterface dialog) {
                ...
                BottomViewPagerBehaviour.from(bottomSheet).setState(BottomViewPagerBehaviour.STATE_COLLAPSED);
                if (viewgroup_static != null) {
                    int range = (int) (metrics.heightPixels - (metrics.density * peek_height) - getStatusBarHeight(getContext()));
        ...
        if (behavior != null && behavior instanceof BottomViewPagerBehaviour) {
            ((BottomViewPagerBehaviour) behavior).setBottomSheetCallback(mBottomSheetBehaviorCallback);
        }

        scale_by = (float) (peek_height * 1.6 / fab_size) * metrics.density;
        fabulous_fab = (FloatingActionButton) contentView.findViewWithTag(FilterConstants.FILTER_FAB_TAG);
        fl = (FrameLayout) contentView.findViewWithTag(FilterConstants.FILTER_FL_TAG);
    ...
    private void fabAnim() {
        ...
        CustomAnimation anim = new CustomAnimation(0, metrics.widthPixels / 2 - fab_pos_x - (fab_size / 2), 0, -(metrics.density * ((peek_height / 2) - ((((metrics.heightPixels - fab_pos_y) - fab_size) / metrics.density)))));
    ...
    public void closeFilter(final Object o) {
        ...
        if (BottomViewPagerBehaviour.from(bottomSheet).getState() == BottomViewPagerBehaviour.STATE_EXPANDED) {
            BottomViewPagerBehaviour.from(bottomSheet).setState(BottomViewPagerBehaviour.STATE_COLLAPSED);
        }
        fabulous_fab.setVisibility(View.VISIBLE);
        view_main.setVisibility(View.INVISIBLE);
        fabulous_fab.animate().scaleXBy(-scale_by)
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        CustomAnimation anim = new CustomAnimation(0, -(metrics.widthPixels / 2 - fab_pos_x - (fab_size / 2)), from_y, to_y);
                        anim.setDuration(anim_duration);
                        fl.startAnimation(anim);
                        anim.setAnimationListener(new Animation.AnimationListener() {
                            @Override
